...
const DEFAULTS = {
  SEND_QUEUE: "image_generation_queue_2",
  CONSUME_QUEUE: "job_results_queue_2",
};
...
class ImageGenerationQueueHandler {
  #lavinMQClient;
  #lavinMQChannel;
  #consumeQueue;
  #sendQueue;
  ...
  constructor(config = {}) {
    if (ImageGenerationQueueHandler.instance) {
      return ImageGenerationQueueHandler.instance;
    }
    ...
    ImageGenerationQueueHandler.instance = this;
  }
  ...
  consumeImageGenerationTasks(callback) {
    ...
    try {
      this.#lavinMQChannel.consume(this.#consumeQueue, callback, {
        noAck: true,
      });
    } catch (error) {
  ...
  async sendJobResult(params) {
    ...
    try {
      ...
      await this.#lavinMQChannel.sendToQueue(
        this.#sendQueue,
        Buffer.from(payload)
      );
...
const imageGenerationQueueHandler = new ImageGenerationQueueHandler({
  consumeQueue: DEFAULTS.CONSUME_QUEUE,
  sendQueue: DEFAULTS.SEND_QUEUE,
});

module.exports = imageGenerationQueueHandler;
